...
const CarFilterControls = ({ filters, currentFilters, onFilterChange, onClearFilter }) => {
    ...
    const filterSections = [
        ...
        },
        {
            id: "transmission",
            title: "Transmission",
            options: filters.transmissions.map((type) => ({ value: type, label: type })),
                ...
                <div key={section.id} className='space-y-3'>
                    ...
                    <div className='flex flex-wrap gap-2 max-h-60 overflow-y-auto pr-1'>
                        {section.options.map((option) => (
                            <Badge key={option.value} variant={section.currentValue === option.value ? "default" : "outline"} className={`cursor-pointer px-3 py-1 ${section.currentValue === option.value ?
                                'bg-blue-100 hover:bg-blue-200 text-blue-900 border-blue-200' : 'bg-white hover:bg-gray-100 text-gray-700'}`}
